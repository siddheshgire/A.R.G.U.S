#!/usr/bin/env python3
"""
A.R.G.U.S. — Complete System Workflow & User Views (.docx Generator)
Converts docs/ARGUS_COMPLETE_WORKFLOW_AND_USER_VIEWS.md into a high-quality,
professionally formatted Microsoft Word (.docx) document spanning ~10 pages.
"""
...
MD_PATH = WORKSPACE_ROOT / "docs" / "ARGUS_COMPLETE_WORKFLOW_AND_USER_VIEWS.md"
DOCX_PATH = WORKSPACE_ROOT / "docs" / "ARGUS_COMPLETE_WORKFLOW_AND_USER_VIEWS.docx"
...
def apply_table_borders(table):
    """Applies clean border styling to table."""
...
def format_runs(p, text, default_size=9, default_color=CHARCOAL, is_bold=False):
    """Helper to parse markdown bold and code inside a text block."""
...
def build_docx():
    ...
    for section in doc.sections:
        ...
        hrun = hp.add_run("A.R.G.U.S. — Complete System Workflow & Operational User Views")
        ...
        frun = fp.add_run("A.R.G.U.S. Comprehensive Architecture • Odd Semester 2026–27 • Department of AIML")
    ...
    r_title = title_p.add_run("A.R.G.U.S. — System Workflow & User Views")
    ...
    r_sub = sub_p.add_run("Comprehensive Operational Blueprint, Access Level Architecture & Interface Specifications")
    ...
    tag_p.paragraph_format.space_after = Pt(10)
    
    meta_runs = [
        ("Academic Year: ", True), ("2026–27 (Odd Semester)  |  ", False),
        ("Programme: ", True), ("B.Tech AIML – C\n", False),
        ("Project: ", True), ("A.R.G.U.S. (Automated Risk Assessment & Anomaly Detection System)\n", False),
        ("Mentor / Guide: ", True), ("Prof. Sunil Kale  |  ", False),
        ("Student Team: ", True), ("Savar Shetty, Pranav Pawar, Siddhesh Gire, Devraj Misal, Atharva Morbale", False),
    ]
    ...
    while i < len(lines):
        ...
        # Skip top-level headers that were already formatted in cover header
        if line.startswith("# A.R.G.U.S. — Complete System") or line.startswith("## Comprehensive Operational") or line.startswith("### Academic Technical"):
            i += 1
            continue
        if line.strip() == "---":
            i += 1
            continue
        if line.startswith("**Academic Year:**") or line.startswith("**Programme") or line.startswith("**Project:") or line.startswith("**Subtitle:") or line.startswith("**Mentor") or line.startswith("**Student Team:"):
            i += 1
            continue
        ...
        if line.startswith("## "):
            ...
            hp.paragraph_format.space_before = Pt(13)
        ...
        if line.startswith("### "):
            h_text = line[4:].strip()
            hp = doc.add_paragraph()
            hp.paragraph_format.space_before = Pt(9)
        ...
        if line.startswith("- ") or line.startswith("* "):
            ...
            format_runs(bp, bullet_text, default_size=9)
            i += 1
            continue
        ...
        if m_num:
            ...
            format_runs(np, num_text, default_size=9)
            i += 1
            continue
        ...
        format_runs(p, line, default_size=9)
